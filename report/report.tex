\documentclass[11pt,a4paper]{article}

\usepackage[margin=2.5cm]{geometry}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}
\usepackage{xcolor}
\usepackage{listings}
\usepackage{hyperref}
\usepackage{float}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,fit,backgrounds}
\usepackage{caption}
\usepackage{enumitem}
\usepackage{fancyhdr}
\usepackage{parskip}
\usepackage{amssymb}

\hypersetup{
  colorlinks=true,
  linkcolor=blue!50!black,
  urlcolor=blue!50!black,
  citecolor=blue!50!black,
  pdftitle={Apache Drill as a Federated Query Engine},
  pdfauthor={Domenico Villari}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Apache Drill as a Federated Query Engine}
\fancyhead[R]{\small \thepage}
\renewcommand{\headrulewidth}{0.3pt}
\definecolor{codebg}{RGB}{247,247,247}
\definecolor{codekw}{RGB}{0,90,150}
\definecolor{codestr}{RGB}{140,20,20}
\definecolor{codecom}{RGB}{100,100,100}
\definecolor{coderule}{RGB}{200,200,200}

\lstdefinestyle{sqlstyle}{
  language=SQL,
  backgroundcolor=\color{codebg},
  basicstyle=\ttfamily\small,
  keywordstyle=\color{codekw}\bfseries,
  stringstyle=\color{codestr},
  commentstyle=\color{codecom}\itshape,
  breaklines=true,
  breakatwhitespace=false,
  frame=single,
  rulecolor=\color{coderule},
  showstringspaces=false,
  tabsize=2,
  columns=fullflexible,
  aboveskip=8pt,
  belowskip=8pt
}

\lstdefinestyle{pystyle}{
  language=Python,
  backgroundcolor=\color{codebg},
  basicstyle=\ttfamily\small,
  keywordstyle=\color{codekw}\bfseries,
  stringstyle=\color{codestr},
  commentstyle=\color{codecom}\itshape,
  breaklines=true,
  breakatwhitespace=false,
  frame=single,
  rulecolor=\color{coderule},
  showstringspaces=false,
  tabsize=4,
  columns=fullflexible,
  aboveskip=8pt,
  belowskip=8pt
}

\lstdefinestyle{yamlstyle}{
  backgroundcolor=\color{codebg},
  basicstyle=\ttfamily\small,
  keywordstyle=\color{codekw}\bfseries,
  breaklines=true,
  breakatwhitespace=false,
  frame=single,
  rulecolor=\color{coderule},
  showstringspaces=false,
  tabsize=2,
  columns=fullflexible,
  morekeywords={version,networks,volumes,services,image,container_name,ports,
    environment,depends_on,healthcheck,test,interval,timeout,retries,command,
    platform,driver,bridge,condition},
  aboveskip=8pt,
  belowskip=8pt
}

\lstdefinestyle{jsonstyle}{
  backgroundcolor=\color{codebg},
  basicstyle=\ttfamily\small,
  stringstyle=\color{codestr},
  breaklines=true,
  breakatwhitespace=false,
  frame=single,
  rulecolor=\color{coderule},
  showstringspaces=false,
  tabsize=2,
  columns=fullflexible,
  aboveskip=8pt,
  belowskip=8pt
}

\lstdefinestyle{bashstyle}{
  language=bash,
  backgroundcolor=\color{codebg},
  basicstyle=\ttfamily\small,
  keywordstyle=\color{codekw}\bfseries,
  breaklines=true,
  breakatwhitespace=false,
  frame=single,
  rulecolor=\color{coderule},
  showstringspaces=false,
  tabsize=2,
  columns=fullflexible,
  aboveskip=8pt,
  belowskip=8pt
}

\title{\Huge\textbf{Apache Drill as a Federated Query Engine}\\[0.4em]
\Large A Hands-On Tutorial over COVID-19 Open Data}
\author{Domenico Villari\\\small Big Data Analytics}
\date{\today}




\begin{document}

\begin{titlepage}
\centering
\vspace*{3cm}
{\Huge\textbf{Apache Drill as a Federated Query Engine}\par}
\vspace{0.6cm}
{\Large  Tutorial over COVID-19 Open Data\par}
\vspace{2cm}
{\large Domenico Villari\par}
{\normalsize Big Data Analytics \par}
\vspace{3cm}
{\normalsize Accademic Year 2025/2026\par}
\vfill

\end{titlepage}

\tableofcontents
\newpage






\section{Introduction}
\label{sec:intro}

\subsection{What Apache Drill Is}

Apache Drill is a distributed SQL query engine designed for \textbf{schema-on-read}
analysis of data. It originated in the \textbf{Hadoop ecosystem}. 
The system isn't a database in the traditional sense: it has no native storage layer of its own.
Instead, it \textbf{compiles a SQL query into a distributed execution plan} and \textbf{push parts of that plan down into whatever \emph{storage plugins} are configured}, a local or distributed filesystem, a JDBC-reachable relational database, a MongoDB cluster, HBase, Kafka, and so on. The engine's job is to make many different systems addressable through one SQL dialect and one execution engine, resolving each table reference and each column's type at query time rather than from a pre-registered catalog.

This positions Drill specifically for \textbf{exploratory and integrative analytics}: connecting
a data scientist's SQL skills directly to whatever storage a dataset already lives in,
without first standing up a data warehouse or writing loader jobs. Two properties make
this practical.
First, \textbf{schema-on-read}: Drill infers column types by sampling the data
at query time, so a CSV file with an irregular header, or a MongoDB collection whose
documents vary from one record to the next, is queryable immediately. 

Second, storage\textbf{ plugins are composable within a single query}: a \texttt{JOIN} clause can freely mix tables that live in entirely different systems, and Drill takes care of scanning each one, pushing down filters and aggregations where the underlying system supports them, and combining the partial results in its own execution engine.

\subsection{The Problem}

Real organizations rarely keep all their data in one place. A typical pipeline has
flat files dropped by an external partner, a relational database serving an
operational application, and a document store holding semi-structured records from a
web service. 
Answering a single analytical question that spans all three normally
requires an \textbf{ETL }step: export each source, reshape it into a common format, and load it
into a data warehouse or a single analytical database before a SQL query can touch all
of it together. That step costs engineering time, so  for small,  exploratory
questions, that cost is disproportionate to the question being asked.

\subsection{Domain: HealthData.gov COVID-19 Data}

The dataset used throughout this tutorial comes from
\textbf{HealthData.gov}, the open-data portal of the U.S.
\textbf{Department of Health \& Human Services (HHS)}. four distinct COVID-19 datasets are
used, described in full in Section~\ref{sec:datasets}: 
\begin{itemize}
    \item state-level hospital capacity
time series
\item  HHS's own statistical estimate of the same phenomenon
\item a locator of
COVID-19 therapeutic dispensing sites
\item  a county-level epidemiological summary
\item a dataset of antiviral treatment availability by site.
\end{itemize}

All four originate from the same
pandemic-response reporting effort but were designed, collected and shaped by
different teams for different purposes, which is exactly why they end up looking so
different from one another: one is a wide, raw, comma-formatted CSV; one is a clean,
regularly-shaped relational table; one is a variable-schema set of records naturally
suited to a document store.



\subsection{What This Tutorial Demonstrates}

Concretely, the tutorial: \begin{enumerate}
    \item \textbf{How to istantiate Drill, PostgreSQL, MongoDB and Jupyter with
Docker Compose};
\item \textbf{loads} the same conceptual \textbf{data} differently depending on its shape: raw CSV on the filesystem, a normalized table in PostgreSQL, unstructured documents in
MongoDB;
\item \textbf{configures three Drill storage plugin}s (\texttt{dfs}, \texttt{pg}, \texttt{mongo})
\item \textbf{runs single-source queries} against each backend to show
schema-on-read, JDBC usage and document-model flexibility individually
\item \textbf{builds up}, step by step, \textbf{a federated query that joins all sources in one
\texttt{SELECT} statement}:  first pre-aggregated by state, then at full row-level
granularity
\end{enumerate}


\section{System Architecture}
\label{sec:architecture}

The entire environment runs as a set of \textbf{Docker containers} on a single machine,
orchestrated with a \texttt{docker-compose.yml} file and connected through a private
bridge network named \texttt{health-net} (the full walkthrough of that file is in
Section~\ref{sec:setup}).
Four services matter for the Drill narrative:\textbf{ PostgreSQL, MongoDB, Apache Drill} itself, and a \textbf{Jupyter notebook }used to drive queries and plot results.

Figure~\ref{fig:architecture} shows how they relate. The key idea to take away from
this diagram is that \textbf{Drill is not a database}: it holds no data of its own.
It \textbf{is a distributed query engine} that reaches \emph{into} \textbf{PostgreSQL over JDBC}, into
\textbf{MongoDB over its native wire protocol}, \textbf{and into a mounted directory of CSV and
Parquet files on the local filesystem}. \\
Each of those three connections is configured
as a Drill \emph{storage plugin}:

\begin{itemize}[leftmargin=1.6em]
 \item \texttt{dfs}: Drill's official name for this plugin type is
    \emph{Distributed File System}, built to also reach HDFS, but
    here it is configured with \texttt{"connection": "file:///"}, a plain local
    directory (\texttt{./data} on the host, mounted at \texttt{/data/healthdata} in
    the Drill container). \textbf{No HDFS and no distributed storage are involved}:
    it is the container's own local disk. It understands multiple file formats: CSV, JSON, Parquet;  so \emph{CSV and Parquet are two formats under the same plugin.}
  \item \texttt{pg}: a generic JDBC plugin pointed at the PostgreSQL instance.
  \item \texttt{mongo}: a native MongoDB plugin that reads BSON documents directly
    and exposes them as SQL rows.
\end{itemize}

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  box/.style={draw, rounded corners, minimum width=2.9cm, minimum height=1cm, align=center, font=\small},
  arrow/.style={-{Latex[length=2.2mm]}, thick}
]
  \node[box, fill=blue!10] (pg) at (0,3.2) {PostgreSQL\\port 5432};
  \node[box, fill=green!10] (mg) at (4.4,3.2) {MongoDB\\port 27017};
  \node[box, fill=orange!20, minimum width=4.6cm] (dr) at (2.2,1.2) {Apache Drill\\port 8047 (UI) / 31010 (JDBC)};
  \node[box, fill=gray!10] (fs) at (2.2,-1.0) {./data (dfs plugin)};
  \node[box, fill=purple!10] (jp) at (7.6,1.2) {Jupyter\\port 8888};

  \draw[arrow] (dr) -- (pg);
  \draw[arrow] (dr) -- (mg);
  \draw[arrow] (dr) -- (fs);
  \draw[arrow] (jp) -- (dr) node[midway, above, font=\scriptsize] {REST API};

  \begin{scope}[on background layer]
    \node[draw, dashed, rounded corners, fit=(pg)(mg)(dr)(fs)(jp), inner sep=0.5cm,
      label={[font=\scriptsize]below left:health-net (Docker bridge network)}] {};
  \end{scope}
\end{tikzpicture}
\caption{Core services of the tutorial environment, all attached to the
\texttt{health-net} Docker bridge network and addressable by hostname. Drill has no
storage engine of its own: it reaches PostgreSQL over JDBC, MongoDB over its native
driver, and local CSV/Parquet files through the \texttt{dfs} plugin. }
%Jupyter nevertalks to PostgreSQL or MongoDB directly , it always goes through Drill's REST API.}
\label{fig:architecture}
\end{figure}

The Docker Compose file used for development also defines two optional
administration UIs, \texttt{pgadmin} (a PostgreSQL admin console on port 5050) and
\texttt{mongo-express} (a MongoDB admin console on port 8081). Neither is essential for the work of Drill.~\ref{fig:architecture}.




\section{The Four Datasets}
\label{sec:datasets}

All four datasets are published by HealthData.gov and describe different facets of
the U.S. COVID-19 response. HealthData.gov exposes them through a Socrata API that
caps unauthenticated responses at 1{,}000 rows, \textbf{so the full files were downloaded
manually from the portal} rather than paginated programmatically (see Section B, for the exact download commands). Table~\ref{tab:datasets}
summarizes size and shape measured; the paragraphs below explain what each
dataset actually records and which columns matter for the queries in this tutorial.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{@{}p{3.2cm}c r r p{5.4cm}@{}}
\toprule
\textbf{Dataset} & \textbf{Plugin} & \textbf{Rows} & \textbf{Cols}  & \textbf{Role} \\
\midrule
1. Hospital Capacity      & \texttt{dfs}   & 81{,}713 & 135 & Raw daily state time series (schema-on-read demo) \\
2. Estimated Beds         & \texttt{pg}    & 1{,}612  & 12   & Regular relational table, HHS's own estimate \\
3. Therapeutic Locator    & \texttt{mongo} & 69{,}184 & 15   & Variable-schema provider records \\
4. Community Profile (county) & \texttt{dfs} & 3{,}294 & 39   & Finer-grained companion to Dataset 1 \\

\bottomrule
\end{tabular}
\caption{The four datasets, their Drill storage plugin, and their raw size as measured
by \texttt{scripts/inspect\_datasets.py}.}
\label{tab:datasets}
\end{table}

\paragraph{Dataset 1: COVID-19 Reported Patient Impact and Hospital Capacity by State.}
The backbone time series of the whole tutorial, queried by almost every SQL statement
in Sections~\ref{sec:basic-queries}--\ref{sec:multiscale}. One row per state per day.
Key columns:
\begin{itemize}[leftmargin=1.6em]
  \item \texttt{state} -- two-letter U.S. state code.
  \item \texttt{date} -- reporting day, stored as a plain \texttt{yyyy/MM/dd} string
    (not a native date type, this matters in Section~\ref{sec:federated-join}).
  \item \texttt{inpatient\_beds\_used\_covid} -- inpatient beds occupied by confirmed
    COVID-19 patients. Raw values use a comma as a thousands separator (e.g.\
    \texttt{"1,338"}) and can be empty, which is why every query that touches this
    column needs an explicit cleaning step.
  \item \texttt{inpatient\_beds}, \texttt{inpatient\_beds\_utilization} -- total
    staffed beds and overall bed utilization, all causes.
  \item \texttt{staffed\_icu\_adult\_patients\_confirmed\_covid} -- adult ICU beds
    occupied by confirmed COVID-19 patients.
\end{itemize}
The remaining $\sim$130 columns break these same measures down further: by age
bracket, by pediatric vs.\ adult, by ventilator use, by influenza co-infection, by
data-coverage flags; and are not used individually in this tutorial;
. This dataset also has a
cleaned, Parquet-converted counterpart (Section~\ref{sec:data-prep}).

\paragraph{Dataset 2: COVID-19 Estimated Inpatient Beds Occupied by State.}
HHS's own statistical estimate of the same phenomenon Dataset~1 reports directly,
which is exactly what makes it interesting to compare against Dataset~1 in
Section~\ref{sec:federated-join}. Regular, stable schema -- state $\times$ date
$\times$ a handful of numeric estimates,  So it was\textbf{ loaded into PostgreSQL} rather
than left as a flat file. Columns, after normalization (Section~\ref{sec:data-prep}):
\begin{itemize}[leftmargin=1.6em]
  \item \texttt{state}, \texttt{collection\_date} -- the latter stored as a genuine
    SQL \texttt{DATE}, unlike Dataset~1's string date.
  \item \texttt{estimated\_beds\_covid} -- modeled estimate of COVID-occupied
    inpatient beds.
  \item \texttt{count\_ll} / \texttt{count\_ul} -- lower/upper bound of that estimate.
  \item \texttt{pct\_beds\_covid}, \texttt{pct\_ll}, \texttt{pct\_ul} -- the same
    estimate expressed as a percentage of capacity, with bounds.
  \item \texttt{total\_inpatient\_beds}, \texttt{total\_ll}, \texttt{total\_ul} --
    estimated total inpatient capacity.
  \item \texttt{geocoded\_state} -- state centroid coordinates, for mapping.
\end{itemize}

\paragraph{Dataset 3: COVID-19 Public Therapeutic Locator.}
One row per (pharmacy or clinic, therapy) pair: which sites dispense which COVID-19
antiviral drug. Fields vary meaningfully across therapy types, which is the reason
\textbf{this dataset was loaded into MongoDB }as a document collection instead of a relational
table.
\begin{itemize}[leftmargin=1.6em]
  \item \texttt{provider\_name}, \texttt{address1/2}, \texttt{city}, \texttt{county},
    \texttt{state}, \texttt{zip} -- site identity and location.
  \item \texttt{national\_drug\_code} -- the NDC code of the specific drug product.
  \item \texttt{order\_label} -- human-readable therapy name, e.g.\ \texttt{"Paxlovid"},
    \texttt{"Renal Paxlovid"}, \texttt{"Lagevrio (molnupiravir)"}.
  \item \texttt{courses\_available} -- treatment courses currently in stock.
  \item \texttt{npi} -- National Provider Identifier.
  \item \texttt{provider\_status} -- \texttt{ACTIVE} or inactive; every query in this
    tutorial filters to \texttt{ACTIVE} sites only.
\end{itemize}

\paragraph{Dataset 4: COVID-19 Community Profile Report (County Level).}
The finer-grained companion to Dataset~1: the same kind of pandemic-pressure signal,
but at county rather than state granularity, and as a weekly rolling summary rather
than a raw daily count. Used in the multi-scale Query~6
(Section~\ref{sec:multiscale}). Representative columns:
\begin{itemize}[leftmargin=1.6em]
  \item \texttt{fips}, \texttt{county}, \texttt{state}, \texttt{fema\_region},
    \texttt{date}.
  \item \texttt{cases\_last\_7\_days}, \texttt{cases\_per\_100k\_last\_7\_days},
    \texttt{total\_cases}, and a week-over-week \texttt{\_pct\_change\_from\_prev\_week}
    variant of each.
  \item The same pattern repeated for \texttt{deaths\_*}, \texttt{test\_positivity\_*},
    and hospitalization pressure (\texttt{confirmed\_covid\_hosp\_*},
    \texttt{pct\_inpatient\_beds\_used\_*}, \texttt{pct\_icu\_beds\_used\_*},
    \texttt{pct\_vents\_used\_*}).
  \item \texttt{pct\_fully\_vacc\_total\_pop}, \texttt{pct\_fully\_vacc\_65\_and\_older}
    -- vaccination coverage.
\end{itemize}


Table~\ref{tab:roles} summarizes, in one place, the fit between each backend and the
kind of data it was given -- the reasoning spelled out per-dataset above, collected
into a single reference.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{@{}l p{4.4cm} p{6.4cm}@{}}
\toprule
\textbf{Backend} & \textbf{Why this shape fits} & \textbf{Dataset(s)} \\
\midrule
\texttt{dfs} & Periodic flat files, no schema to declare, exactly as delivered by the source & Dataset~1 (Hospital Capacity, raw + numbers-cleaned), Dataset~4 (Community Profile) \\
\texttt{pg}  & Regular, stable schema (state $\times$ date $\times$ a handful of numeric estimates); benefits from a typed table and indexed lookups & Dataset~2 (Estimated Beds) \\
\texttt{mongo} & Fields vary by therapy type; one record per (site, drug) pair, naturally irregular & Dataset~3 (Therapeutic Locator) \\
\bottomrule
\end{tabular}
\caption{Why each dataset ended up on a specific backend}
\label{tab:roles}
\end{table}




\newpage
\section{Environment Setup}
\label{sec:setup}

\subsection{Prerequisites}
The only prerequisite is \textbf{Docker} (verified with \texttt{docker --version}).
Everything else: Drill, PostgreSQL, MongoDB, Jupyter,  is provisioned automatically
by Docker Compose.
The one artifact that has to be fetched \emph{before} the first
\texttt{docker-compose up}, so that the volume mount in
Section~\ref{sec:setup}'s Drill service has something to mount, is the PostgreSQL
JDBC driver:

\begin{lstlisting}[style=bashstyle, caption={Fetching the PostgreSQL JDBC driver (from the project root).}]
curl -L -o jdbc-drivers/postgresql-42.7.3.jar \
  https://repo1.maven.org/maven2/org/postgresql/postgresql/42.7.3/postgresql-42.7.3.jar
\end{lstlisting}

\subsection{docker-compose.yml}
The compose file defines a bridge network \texttt{health-net}, two named volumes for
database persistence, and the services introduced in Section~\ref{sec:architecture}.
The two services most specific to Drill's setup are shown below in full.

\begin{lstlisting}[style=yamlstyle, caption={PostgreSQL service, including the mount that seeds its schema on first boot.}]
 postgres:
    image: postgres:15
    container_name: postgres
    networks:
      - health-net
    ports:
      - "5432:5432"
    environment:
      POSTGRES_DB: ${POSTGRES_DB:-healthdata}
      POSTGRES_USER: ${POSTGRES_USER:-drill}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-drill123}
    volumes:
      - postgres-data:/var/lib/postgresql/data
      - ./init-sql:/docker-entrypoint-initdb.d
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-drill} -d ${POSTGRES_DB:-healthdata}"]
      interval: 10s
      timeout: 5s
      retries: 5
\end{lstlisting}

\begin{lstlisting}[style=yamlstyle, caption={Mongo service, including the mount on first boot}]
mongo:
    image: mongo:6
    container_name: mongo
    networks:
      - health-net
    ports:
      - "27017:27017"
    environment:
      MONGO_INITDB_ROOT_USERNAME: ${MONGO_USER:-drill}
      MONGO_INITDB_ROOT_PASSWORD: ${MONGO_PASSWORD:-drill123}
      MONGO_INITDB_DATABASE: ${MONGO_DB:-healthdata}
    volumes:
      - mongo-data:/data/db
      - ./init-mongo:/docker-entrypoint-initdb.d
    healthcheck:
      test: ["CMD", "mongosh", "--quiet", "--eval", "db.adminCommand('ping').ok"]
      interval: 10s
      timeout: 5s
      retries: 5
\end{lstlisting}


\begin{lstlisting}[style=yamlstyle, caption={Drill service: it depends on Postgres and MongoDB being healthy first, mounts the data directory, the storage-plugin override file, and  the PostgreSQL JDBC driver.}]
drill:
    image: apache/drill:1.21.0
    platform: linux/amd64
    container_name: drill
    networks:
      - health-net
    ports:
      - "8047:8047"   # Web UI
      - "31010:31010" # JDBC / user port
    depends_on:
      postgres:
        condition: service_healthy
      mongo:
        condition: service_healthy
    volumes:
      - ./data:/data/healthdata
      - ./drill-config/storage-plugins.conf:/opt/drill/conf/storage-plugins-override.conf
      - ./jdbc-drivers/postgresql-42.7.3.jar:/opt/drill/jars/3rdparty/postgresql-42.7.3.jar
    stdin_open: true
    environment:
      DRILL_HEAP: "4G"
      DRILL_MAX_DIRECT_MEMORY: "8G"
\end{lstlisting}



The \textbf{PostgreSQL JDBC driver jar} (downloaded once with \texttt{curl} into
\texttt{jdbc-drivers/}) is mounted \emph{directly} into
\texttt{/opt/drill/jars/3rdparty/}  it is available the moment the Drill
container starts. \texttt{Jupyter} follows the same pattern: image,
network and port.
Two more services, \texttt{pgadmin} (port 5050) and \texttt{mongo-express} (port
8081), are optional admin UIs for browsing PostgreSQL/MongoDB visually; they play no
role in the Drill queries themselves and can be omitted from a minimal setup.

\subsection{Starting the Stack}
\begin{lstlisting}[style=bashstyle]
docker-compose up -d
docker-compose ps
\end{lstlisting}
All services should report \texttt{healthy} (Postgres, Mongo) or \texttt{running}
(Drill, Jupyter) within roughly a minute. The very first sanity check is a query
against Drill's own system table, run from the Web UI at
\href{http://localhost:8047}{\texttt{localhost:8047}} or, as in this tutorial's
notebook, over the REST API:
\begin{lstlisting}[style=sqlstyle]
SELECT * FROM sys.version;
\end{lstlisting}
A healthy Drill instance returns a single row reporting version \texttt{1.21.0} and
its build commit id (visible in Section~\ref{sec:notebook}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{img/drill.png}
    \caption{Drill Web UI, Storage page (localhost:8047/storage), showing the
three enabled plugins – dfs, pg, mongo.}
\end{figure}





\section{Data Preparation}
\label{sec:data-prep}

Not every dataset is prepared the same way, and that is a deliberate part of the
tutorial's design. The \textbf{raw} \texttt{covid/hospital\_capacity.csv} (Dataset~1)
is kept exactly as downloaded, messy headers and comma-formatted numbers included,
specifically so Query~1 (Section~\ref{sec:basic-queries}) \textbf{can demonstrate
schema-on-read}: cleaning happens inside that one SQL statement.  Every other query
that touches Dataset~1, though, reads a second, numbers-cleaned copy of the same
file (\texttt{hospital\_capacity\_clean.csv}) produced once upstream, so they are not
forced to repeat that cleaning trick just to do a plain \texttt{SUM}. \textbf{
Dataset~4} stays fully raw on \texttt{dfs} throughout. \textbf{Datasets~2, 3, by
contrast, are normalized \emph{before} loading} into PostgreSQL and MongoDB,
because neither benefits from messy, space-containing column headers the way a
schema-on-read demo does.

\subsection{Obtaining the Raw Datasets}
\label{sec:download-datasets}
Unlike the JDBC driver above, the four datasets \textbf{cannot} be fetched with a
plain \texttt{curl} against a fixed URL: HealthData.gov exposes them through the
Socrata Open Data API, which caps unauthenticated responses at 1{,}000 rows, far
from the 81{,}713 rows in Dataset~1 alone. Reproducing this project therefore
starts \textbf{with a manual step}:\textbf{ open each dataset's page in a browser and use the
portal's \emph{Export~$\rightarrow$~CSV} (or \emph{Download}) button to obtain the
full file, saving all four into \texttt{data/} at the project root.}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{img/download1.png}
    \caption{Manual Dataset's Download}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{img/download2.png}
\end{figure}

\begin{lstlisting}[style=bashstyle, caption={The four HealthData.gov dataset pages (commands.txt, Section B).}]
# Dataset 1 -- Hospital Capacity, State Timeseries (RAW)
https://healthdata.gov/Hospital/COVID-19-Reported-Patient-Impact-and-Hospital-Capa/g62h-syeh/explore/page/filter

# Dataset 2 -- Estimated Inpatient Beds, State Timeseries
https://healthdata.gov/dataset/COVID-19-Estimated-Inpatient-Beds-Occupied-by-COVI/py8k-j5rq/about_data

# Dataset 3 -- Public Therapeutic Locator
https://healthdata.gov/Health/COVID-19-Public-Therapeutic-Locator/rxn6-qnx8/about_data

# Dataset 4 -- Community Profile Report, County Level
https://healthdata.gov/dataset/COVID-19-Community-Profile-Report-County-Level/di4u-7yu6/about_data


\end{lstlisting}

Once the four raw CSVs are sitting in \texttt{data/} (each with a date suffix from
the download, e.g.\ \texttt{\_20260617}), a second script-free step organizes them
into the clean per-dataset subdirectories every later section assumes:

\begin{lstlisting}[style=bashstyle, caption={Organizing the raw downloads (commands.txt, Section B, Step 07b).}]
DATE=20260617   # adjust to the date suffix in your downloaded filenames
mkdir -p data/raw data/covid data/covid_parquet data/county \
         data/estimated_beds data/therapeutic data/treatments

cp "data/COVID-19_Reported_Patient_Impact_and_Hospital_Capacity_by_State_Timeseries_(RAW)_${DATE}.csv" \
   data/covid/hospital_capacity.csv

cp "data/COVID-19_Estimated_Inpatient_Beds_Occupied_by_COVID-19_Patients_by_State_Timeseries_${DATE}.csv" \
   data/estimated_beds/estimated_beds.csv

cp "data/COVID-19_Public_Therapeutic_Locator_${DATE}.csv" \
   data/therapeutic/therapeutic_locator.csv

cp "data/COVID-19_Community_Profile_Report_-_County-Level_${DATE}.csv" \
   data/county/community_profile.csv

cp "data/COVID-19_Treatments_${DATE}.csv" \
   data/treatments/treatments.csv

mv data/COVID-19_*.csv data/raw/   # keep the untouched originals for reference
\end{lstlisting}

Finally, \texttt{scripts/inspect\_datasets.py} documents the result:  exactly the
row/column/size numbers quoted in Table~\ref{tab:datasets}:

\begin{lstlisting}[style=bashstyle]
python3 scripts/inspect_datasets.py 
\end{lstlisting}

\subsection{Column Normalization}
\texttt{scripts/normalize\_columns.py} \textbf{renames the mixed-case, space-containing raw
headers of Datasets~2, 3 and~5} to \texttt{snake\_case} using three hard-coded mapping
dictionaries, and writes out \texttt{*\_clean.csv} files (e.g.\
\texttt{estimated\_beds\_clean.csv}). For Dataset~2 specifically, it also strips the
thousands-separator commas from nine numeric fields before they reach PostgreSQL, so
that the \texttt{COPY} step in Section~\ref{sec:loading} can load them straight into
\texttt{NUMERIC} columns without any further casting. A fourth job runs over
Dataset~1: it leaves every column name untouched (headers stay exactly as
downloaded) and strips commas from just the one numeric column every join query
needs, \texttt{inpatient\_beds\_used\_covid}, writing the result to
\texttt{hospital\_capacity\_clean.csv} alongside the untouched original.
\begin{lstlisting}[style=bashstyle]
python3 scripts/normalize_columns.py
\end{lstlisting}

\subsection{CSV to Parquet}
\texttt{scripts/csv\_to\_parquet.py} converts Dataset~1's raw CSV into a Parquet file, using \texttt{pandas} and \texttt{pyarrow}. Before
writing, it coerces four numeric columns\\
(\texttt{inpatient\_beds\_used\_covid}, \texttt{inpatient\_beds},
\texttt{adult\_icu\_beds\_used}, \\ \texttt{staffed\_icu\_beds\_utilization}) by
stripping thousands-separator commas and calling \\ \texttt{pd.to\_numeric(errors=
"coerce")}, exactly the transformation that Section~\ref{sec:basic-queries}'s Query~1
performs \emph{inside SQL} against the raw CSV. The result is 81{,}713 rows and 135
columns compressed from 55.51~MB to roughly 15.4~MB, a
$\sim$3.6$\times$ reduction. This artifact is available on the \texttt{dfs} plugin at
\texttt{covid\_parquet/hospital\_capacity.parquet}.
\begin{lstlisting}[style=bashstyle]
python3 scripts/csv_to_parquet.py
\end{lstlisting}




\section{Loading Data into PostgreSQL and MongoDB}
\label{sec:loading}

\subsection{PostgreSQL: Dataset 2}
\texttt{init-sql/01\_load\_estimated\_beds.sql} is mounted into
\texttt{/docker-entrypoint-initdb.d}, so PostgreSQL executes it automatically on
first boot. It only defines the schema:

\begin{lstlisting}[style=sqlstyle, caption={init-sql/01\_load\_estimated\_beds.sql -- schema definition for Dataset 2.}]
CREATE SCHEMA IF NOT EXISTS healthdata;

DROP TABLE IF EXISTS healthdata.estimated_beds;

CREATE TABLE healthdata.estimated_beds (
    state                 CHAR(2),
    collection_date       DATE,
    estimated_beds_covid  NUMERIC,
    count_ll              NUMERIC,
    count_ul              NUMERIC,
    pct_beds_covid        NUMERIC,
    pct_ll                NUMERIC,
    pct_ul                NUMERIC,
    total_inpatient_beds  NUMERIC,
    total_ll              NUMERIC,
    total_ul              NUMERIC,
    geocoded_state        TEXT
);
\end{lstlisting}

Loading the actual rows is a separate, manual step: copy the cleaned CSV into the running container, then run a
\texttt{psql~\textbackslash copy}:

\begin{lstlisting}[style=bashstyle]
docker cp data/estimated_beds/estimated_beds_clean.csv \
  postgres:/tmp/estimated_beds_clean.csv

docker exec postgres psql -U drill -d healthdata -c \
  "\copy healthdata.estimated_beds FROM '/tmp/estimated_beds_clean.csv' CSV HEADER;"

docker exec postgres psql -U drill -d healthdata -c \
  "SELECT COUNT(*), MIN(collection_date), MAX(collection_date)
   FROM healthdata.estimated_beds;"
\end{lstlisting}

\subsection{MongoDB: Dataset 3}
Unlike a typical \texttt{mongoimport} shell load, \textbf{Dataset~3 is loaded by a small
Python script}, \texttt{init-mongo/01\_load\_therapeutic.py}, using \texttt{pymongo}.
It reads \texttt{MONGO\_URI}/\texttt{MONGO\_DB} from the environment, drops the
\texttt{therapeutic} collection if it exists, streams
\texttt{therapeutic\_locator\_clean.csv} through \texttt{csv.DictReader}, converts
\texttt{courses\_available} to a float where present, inserts every document with a
single \texttt{insert\_many}, and finally creates three indexes:
\texttt{idx\_state} on \texttt{state}, \texttt{idx\_status} on
\texttt{provider\_status}, \texttt{idx\_order} on \texttt{order\_label}, to keep the
filtered/grouped queries in Section~\ref{sec:basic-queries} fast.

\begin{lstlisting}[style=bashstyle]
python3 init-mongo/01_load_therapeutic.py
\end{lstlisting}



\section{Configuring Drill Storage Plugins}
\label{sec:plugins}

Storage plugins can be configured through the Web UI (\emph{Storage} tab), but for
reproducibility this project keeps the configuration in a file,
\texttt{drill-config/storage-plugins.conf}, mounted into the container as
\texttt{/opt/drill/conf/storage-plugins-override.conf} and applied automatically on
startup. \textbf{The filename matters}: Drill only looks for an override file named
exactly \texttt{storage-plugins-override.conf} (HOCON format, a superset of JSON); mounting it as \texttt{.json} instead, as
an earlier revision of this project did, means Drill silently ignores it and boots
with only its own default \texttt{cp}/\texttt{dfs} plugins, no error logged anywhere.

\begin{lstlisting}[style=jsonstyle, caption={drill-config/storage-plugins.conf -- the three plugins used throughout this tutorial.}]
{
  "storage": {
    "dfs": {
      "type": "file",
      "connection": "file:///",
      "workspaces": {
        "healthdata": {
          "location": "/data/healthdata",
          "writable": true,
          "defaultInputFormat": null
        }
      },
      "formats": {
        "csv":     { "type": "text", "extensions": ["csv"], "extractHeader": true, "delimiter": "," },
        "json":    { "type": "json", "extensions": ["json"] },
        "parquet": { "type": "parquet" }
      },
      "enabled": true
    },
    "pg": {
      "type": "jdbc",
      "driver": "org.postgresql.Driver",
      "url": "jdbc:postgresql://postgres:5432/healthdata",
      "username": "drill",
      "password": "drill123",
      "enabled": true
    },
    "mongo": {
      "type": "mongo",
      "connection": "mongodb://drill:drill123@mongo:27017/",
      "enabled": true
    }
  }
}
\end{lstlisting}

Note that all three plugins reach the other containers \emph{by hostname}
(\texttt{postgres}, \texttt{mongo}) rather than by IP, Docker's embedded DNS on the
\texttt{health-net} bridge network resolves those names automatically. Once the
container restarts with this file in place, every source can be tested
independently:

\begin{lstlisting}[style=sqlstyle]
SELECT * FROM dfs.healthdata.`covid/hospital_capacity.csv` LIMIT 5;
SELECT * FROM pg.healthdata.estimated_beds LIMIT 5;
SELECT * FROM mongo.healthdata.therapeutic LIMIT 5;
\end{lstlisting}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{img/query2.png}
    \caption{Drill Web UI: a query executed against the \texttt{mongo} plugin.}
\label{fig:drill-ui-query}
\end{figure}




\section{Basic Federated Queries}
\label{sec:basic-queries}

With all three plugins verified, the tutorial first queries each source
\emph{individually}, before combining
them in Section~\ref{sec:federated-join}. All SQL below, and all timings quoted, are
taken verbatim from the accompanying \texttt{notebooks/analysis.ipynb}
(Section~\ref{sec:notebook}). Table~\ref{tab:queryset} gives the full roadmap before
the per-query detail below: Q1--Q3 each touch one backend; QP is a single-row point
lookup rather than an aggregation, added specifically to cover a query shape none of
Q1--Q3 exercise; Q4a--Q4c join every possible pair among the three backends,
pre-aggregated; Q4d joins all three, still pre-aggregated; Q4f repeats the
\texttt{dfs}+\texttt{pg} pair from Q4a at row-level granularity instead; Q6 joins two
files on the same backend (\texttt{dfs}).

\begin{table}[htbp]
\centering
\small
\begin{tabular}{@{}l p{2.6cm} p{9.2cm}@{}}
\toprule
\textbf{ID} & \textbf{Shape} & \textbf{Query} \\
\midrule
Q1  & aggregation + top-$N$          & COVID beds in use per state and day, top 20 (raw CSV, schema-on-read) \\
Q2  & aggregation                    & Avg.\ estimated COVID bed occupancy per state (\texttt{pg}) \\
Q3  & aggregation + filter           & Active therapy centers per state and drug (\texttt{mongo}) \\
QP  & point lookup                   & Single (state, date) row, exact match on both predicates (\texttt{pg}) \\
Q4a & 2-way join, cross-backend      & \texttt{dfs} $\bowtie$ \texttt{pg}, pre-aggregated by state \\
Q4b & 2-way join, cross-backend      & \texttt{dfs} $\bowtie$ \texttt{mongo}, pre-aggregated by state \\
Q4c & 2-way join, cross-backend      & \texttt{pg} $\bowtie$ \texttt{mongo}, pre-aggregated by state \\
Q4d & 3-way join, cross-backend      & \texttt{dfs} $\bowtie$ \texttt{pg} $\bowtie$ \texttt{mongo}, pre-aggregated by state ( ``key query'') \\
Q4f & row-level join, cross-backend  & Q4a's pair (\texttt{dfs} $\bowtie$ \texttt{pg}) at row-level, on state~+~date, enriched with \texttt{mongo}'s aggregate \\
Q6  & join, same backend             & \texttt{dfs} $\bowtie$ \texttt{dfs} (two files, state vs.\ county granularity) \\
\bottomrule
\end{tabular}
\caption{Full query set. Q1--Q3 touch one backend each; QP is a point lookup, the
one shape none of the aggregation queries cover; Q4a--Q4c cover all three
possible cross-backend pairs, pre-aggregated; Q4d is the 3-way pre-aggregated join;
Q4f re-runs Q4a's exact pair at row-level granularity to isolate the cost of
pre-aggregation from the cost of federation itself; Q6 joins within a single
backend as a control.}
\label{tab:queryset}
\end{table}

\subsection{Query 1 -- Schema-on-Read on Raw CSV (\texttt{dfs})}
\begin{lstlisting}[style=sqlstyle]
SELECT state, `date`, covid_beds_used FROM (
  SELECT state,
         `date`,
         SUM(CAST(NULLIF(REPLACE(inpatient_beds_used_covid, ',', ''), '') AS BIGINT)) AS covid_beds_used
  FROM dfs.healthdata.`covid/hospital_capacity.csv`
  WHERE `date` >= '2021/01/01'
  GROUP BY state, `date`
)
WHERE covid_beds_used IS NOT NULL
ORDER BY covid_beds_used DESC
LIMIT 20
\end{lstlisting}
Three things are worth pointing out here, because none of them require any
preprocessing outside the query itself. \begin{itemize}
    \item First, \texttt{date} is a Drill reserved
word and must be back-ticked.
\item  Second, \texttt{REPLACE(\ldots, ',', '')} strips the
thousands separator before \texttt{NULLIF} turns empty strings into SQL \texttt{NULL}
and \texttt{CAST} converts the result to \texttt{BIGINT}, this is \textbf{schema-on-read }in
practice: the raw text column is coerced to a number \emph{inside} the query, on the
fly.
\item Third, the outer \texttt{WHERE covid\_beds\_used IS NOT NULL} is not redundant:
Drill sorts \texttt{NULL} as larger than any number under a default descending
\texttt{ORDER BY}, so rows with missing data would otherwise appear \emph{first},
ahead of every real value.
\end{itemize}  This query returns 20 rows in a median 1.02\,s over 3 runs.

\subsection{Query 1b -- Same Data, Parquet Instead of CSV (\texttt{dfs})}
\label{sec:q1b}
Section~\ref{sec:architecture} claims that \texttt{dfs} reads CSV and Parquet as
two formats under the \emph{same} plugin. Query~1b makes that claim concrete: the
exact same aggregation as Query~1, against the exact same underlying data, but
reading \texttt{covid\_parquet/hospital\_capacity.parquet} instead of the raw CSV.
\begin{lstlisting}[style=sqlstyle]
SELECT state, `date`, covid_beds_used FROM (
  SELECT state,
         `date`,
         SUM(inpatient_beds_used_covid) AS covid_beds_used
  FROM dfs.healthdata.`covid_parquet/hospital_capacity.parquet`
  WHERE `date` >= '2021/01/01'
  GROUP BY state, `date`
)
WHERE covid_beds_used IS NOT NULL
ORDER BY covid_beds_used DESC
LIMIT 20
\end{lstlisting}
Two things stand out. First, the query itself is shorter: the Parquet file was
produced once, upstream, by \texttt{scripts/csv\_to\_parquet.py}
(Section~\ref{sec:data-prep}), which also converts \texttt{inpatient\_beds\_used\_covid}
to a real numeric column -- so none of Query~1's \texttt{REPLACE}/\texttt{NULLIF}/\texttt{CAST}
cleanup is needed here; Parquet carries its own schema, CSV does not. Second, it is
faster: a median 0.60\,s over 3 runs versus Query~1's 1.02\,s, for byte-identical
results (the same 20 rows, same order -- Parquet's numeric column simply renders as
a float rather than a cast \texttt{BIGINT}). Drill's own query plan explains why: the
Parquet scan's estimated I/O cost is roughly 240 times lower than the CSV scan's,
because Parquet is columnar and compressed (16\,MB versus the CSV's 58\,MB for the
same rows) and lets Drill skip straight to the one column it needs instead of
parsing every field of every row as text.
\subsection{Query 2 -- Aggregation on PostgreSQL (\texttt{pg})}
\begin{lstlisting}[style=sqlstyle]
SELECT state,
       COUNT(*) AS num_observations,
       ROUND(AVG(estimated_beds_covid), 0) AS avg_estimated_beds_covid,
       ROUND(AVG(pct_beds_covid), 4)       AS avg_pct_covid
FROM pg.healthdata.estimated_beds
GROUP BY state
ORDER BY avg_estimated_beds_covid DESC
LIMIT 20
\end{lstlisting}
Here Drill behaves as a JDBC client: because \texttt{estimated\_beds\_covid} is
already a proper \texttt{NUMERIC} column (Section~\ref{sec:data-prep}'s cleaning), no
casting is needed, and Drill can push part of the aggregation down to PostgreSQL's
own query planner rather than computing it entirely in Drill's execution engine. \\
20
rows, a median 1.29\,s over 3 runs.

\subsection{Query 3 -- Schema Flexibility on MongoDB (\texttt{mongo})}
\begin{lstlisting}[style=sqlstyle]
SELECT t.state,
       t.order_label AS therapy_name,
       COUNT(*)      AS num_centers
FROM mongo.healthdata.therapeutic t
WHERE t.provider_status = 'ACTIVE'
GROUP BY t.state, t.order_label
ORDER BY num_centers DESC
LIMIT 20
\end{lstlisting}
MongoDB is a document store: Drill reads each JSON-like document in the
\texttt{therapeutic} collection and projects it as a SQL row, with no relational
schema declared anywhere upfront, the table alias \texttt{t} and dot-notation
(\texttt{t.state}, \texttt{t.order\_label}) are how Drill lets a query reach into
document fields as if they were ordinary columns. \\
20 rows, a median 0.25\,s over 3 runs, the fastest of the three aggregation queries
(Q1--Q3), helped by the \texttt{idx\_status} index created in
Section~\ref{sec:loading}.

\subsection{Query P -- Point Lookup (\texttt{pg})}
Q1--Q3 are all aggregations: a full (or filtered) scan followed by a
\texttt{GROUP BY}. None of them show what Drill does with the query shape that
benefits most from an index -- a single, highly selective row lookup. Query~P adds
exactly that:
\begin{lstlisting}[style=sqlstyle]
SELECT * FROM pg.healthdata.estimated_beds
WHERE state = 'CA' AND collection_date = DATE '2021-07-09'
\end{lstlisting}
This returns exactly 1 row, in a median 0.21\,s over 3 runs, comparable to Q3 and
markedly faster than Q1's full-CSV aggregation, even though \texttt{estimated\_beds}
carries no explicit index beyond its columns. The comparison is honest rather than
dramatic: with roughly 82{,}000 rows in the underlying tables, even a linear scan
inside PostgreSQL is fast enough that a two-predicate filter barely registers, and
Section~\ref{sec:notebook}'s planning/queued/execution breakdown
(Figure~\ref{fig:timing-breakdown}) shows why, for a query this cheap, Drill's own
query \emph{planning} is the larger share of the total time, not execution against
the backend.


\section{Building the Federated Join Incrementally}
\label{sec:federated-join}

\textbf{The core claim of this tutorial is that Drill can join all three sources in a
\emph{single} SQL statement}. The notebook builds it up incrementally, so that the cost of each additional piece of
complexity can be measured on its own. From this point on, every query reads
\texttt{covid/hospital\_capacity\_clean.csv} instead of the raw file: the thousands-separator
commas in \texttt{inpatient\_beds\_used\_covid} are already gone
(Section~\ref{sec:data-prep}), so none of them need Query~1's \texttt{REPLACE} any
more. A short \texttt{CAST(NULLIF(\ldots, ''), ...)} still wraps every read of that
column, though: 846 of the 81{,}713 rows have it genuinely empty (not comma-dirty,
just missing), and Drill's CSV reader has no concept of \texttt{NULL} for an empty
cell -- an aggregate like \texttt{SUM} would fail outright the moment it hit one
without that guard. Each subquery is also written as a
\texttt{WITH} common table expression (CTE) rather than nested inside \texttt{FROM
(\ldots)}: Drill supports \texttt{WITH} natively, and reading a join top to bottom is
easier than unwinding nested parentheses.

\subsection{2-Way Joins, Pre-Aggregated by State}
Queries~4a--4c each pre-aggregate two sources down to one row per state \emph{before}
joining them -- so the join itself only ever combines a handful of small,
already-summarized row sets.

\begin{lstlisting}[style=sqlstyle, caption={Q4a -- dfs + pg, 2-way, pre-aggregated by state.}]
WITH h AS (
    SELECT state, SUM(CAST(NULLIF(inpatient_beds_used_covid, '') AS BIGINT)) AS covid_beds_used
    FROM dfs.healthdata.`covid/hospital_capacity_clean.csv`
    WHERE `date` = '2021/07/09'
    GROUP BY state
),
e AS (
    SELECT state, ROUND(AVG(estimated_beds_covid), 0) AS avg_estimated_beds_covid
    FROM pg.healthdata.estimated_beds
    GROUP BY state
)
SELECT h.state, h.covid_beds_used, e.avg_estimated_beds_covid
FROM h
JOIN e ON h.state = e.state
ORDER BY h.covid_beds_used DESC
\end{lstlisting}

Queries~4b (\texttt{dfs}+\texttt{mongo}) and~4c (\texttt{pg}+\texttt{mongo}) follow
exactly the same shape: two CTEs, each \texttt{GROUP BY state} against a single
plugin, joined \texttt{ON} state. Only the pair of sources -- and, for
\texttt{mongo}, the extra \texttt{WHERE provider\_status = 'ACTIVE'} filter -- changes
between them.
\begin{lstlisting}[style=sqlstyle, caption={Query 4b (dfs+mongo) }]
WITH h AS (
    SELECT state, SUM(CAST(NULLIF(inpatient_beds_used_covid, '') AS BIGINT)) AS covid_beds_used
    FROM dfs.healthdata.`covid/hospital_capacity_clean.csv`
    WHERE `date` = '2021/07/09'
    GROUP BY state
),
c AS (
    SELECT t.state, COUNT(*) AS num_therapy_centers
    FROM mongo.healthdata.therapeutic t
    WHERE t.provider_status = 'ACTIVE'
    GROUP BY t.state
)
SELECT h.state, h.covid_beds_used, c.num_therapy_centers
FROM h
JOIN c ON h.state = c.state
ORDER BY h.covid_beds_used DESC
\end{lstlisting}

\begin{lstlisting}[style=sqlstyle, caption={Query 4c (pg+mongo) }]
WITH e AS (
    SELECT state, ROUND(AVG(estimated_beds_covid), 0) AS avg_estimated_beds_covid
    FROM pg.healthdata.estimated_beds
    GROUP BY state
),
c AS (
    SELECT t.state, COUNT(*) AS num_therapy_centers
    FROM mongo.healthdata.therapeutic t
    WHERE t.provider_status = 'ACTIVE'
    GROUP BY t.state
)
SELECT e.state, e.avg_estimated_beds_covid, c.num_therapy_centers
FROM e
JOIN c ON e.state = c.state
ORDER BY e.avg_estimated_beds_covid DESC
\end{lstlisting}

\subsection{3-Way Join, Still Pre-Aggregated}
Query~4d extends the same pattern to all three sources at once:

\begin{lstlisting}[style=sqlstyle, caption={Q4d -- dfs + pg + mongo, 3-way, pre-aggregated by state.}]
WITH h AS (
    SELECT state, SUM(CAST(NULLIF(inpatient_beds_used_covid, '') AS BIGINT)) AS covid_beds_used
    FROM dfs.healthdata.`covid/hospital_capacity_clean.csv`
    WHERE `date` = '2021/07/09'
    GROUP BY state
),
e AS (
    SELECT state, ROUND(AVG(estimated_beds_covid), 0) AS avg_estimated_beds_covid
    FROM pg.healthdata.estimated_beds
    GROUP BY state
),
c AS (
    SELECT t.state, COUNT(*) AS num_therapy_centers
    FROM mongo.healthdata.therapeutic t
    WHERE t.provider_status = 'ACTIVE'
    GROUP BY t.state
)
SELECT h.state, h.covid_beds_used, e.avg_estimated_beds_covid, c.num_therapy_centers
FROM h
JOIN e ON h.state = e.state
JOIN c ON h.state = c.state
ORDER BY h.covid_beds_used DESC
\end{lstlisting}

This single \texttt{SELECT} answers a research question: for every state, it reports reported COVID beds in
use (CSV), the HHS statistical estimate of the same (PostgreSQL), and the number of
active therapeutic centers (MongoDB), \textbf{three systems, one query, zero ETL.}

\subsection{Row-Level Join}
Queries~4a--4d all pre-aggregate before joining, which keeps the join itself cheap:
each side contributes at most 51 rows (one per state). Query~4f instead joins
\texttt{dfs} and \texttt{pg} \emph{row by row}, on the composite key
\texttt{state~+~date}, with no pre-aggregation on either side, comparing real,
reported hospital occupancy against the statistical estimate on a specific day, for
every state, and then enriching the result with \texttt{mongo}'s aggregated
therapy-center counts:

\begin{lstlisting}[style=sqlstyle, caption={Q4f -- dfs + pg row-level join (on state + date), enriched with mongo.}]
WITH hp AS (
    SELECT h.state, h.`date`, CAST(NULLIF(h.inpatient_beds_used_covid, '') AS BIGINT) AS covid_beds_used,
           e.estimated_beds_covid
    FROM dfs.healthdata.`covid/hospital_capacity_clean.csv` h
    JOIN pg.healthdata.estimated_beds e
      ON h.state = e.state
     AND TO_DATE(h.`date`, 'yyyy/MM/dd') = e.collection_date
),
c AS (
    SELECT t.state, COUNT(*) AS num_therapy_centers
    FROM mongo.healthdata.therapeutic t
    WHERE t.provider_status = 'ACTIVE'
    GROUP BY t.state
)
SELECT hp.state, hp.`date`, hp.covid_beds_used, hp.estimated_beds_covid, c.num_therapy_centers
FROM hp
JOIN c ON hp.state = c.state
ORDER BY hp.state, hp.`date`
\end{lstlisting}

Two things set this query apart. \textbf{First, the join condition bridges two date
representations}: \texttt{dfs}'s \texttt{date} is a plain \texttt{VARCHAR}
(\texttt{yyyy/MM/dd}), \texttt{pg}'s \texttt{collection\_date} a native \texttt{DATE},
so the predicate needs \texttt{TO\_DATE(h.\`date\`, 'yyyy/MM/dd')}. \textbf{Second, neither side is
pre-aggregated}: the join matches at full row-level granularity, returning
1{,}581 rows instead of 51, at a median 2.52\,s over 3 runs: by far the slowest
query in the tutorial (Figure~\ref{fig:join-cost}). 

This also separates two
independent axes: \emph{3-way} (how many sources) vs.\ \emph{row-level} (what
granularity). Query~4d is 3-way but pre-aggregated; Query~4f is only 2-way at
row-level (\texttt{dfs}+\texttt{pg}), with \texttt{mongo} folded in afterward, still
aggregated. It is the combination of more sources \emph{and} finer granularity that
makes a federated join expensive, as Figure~\ref{fig:join-cost} shows.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{img/top10_states.png}
\caption{Top 10 states by reported COVID beds in use, from the 3-way federated
Query~4d.}
\label{fig:top10-states}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{img/join_incremental_cost.png}
\caption{Execution time as join complexity grows: three 2-way joins (Q4a--Q4c),
one 3-way pre-aggregated join (Q4d), and the row-level join (Q4f). Row count is
annotated on each bar -- note Q4f's jump from 51 to 1{,}581 rows.}
\label{fig:join-cost}
\end{figure}


\section{Multi-Scale Query}
\label{sec:multiscale}

Not every useful join is federated across storage systems. Query~6 joins two
\emph{different files on the same \texttt{dfs} plugin} -- Dataset~1 (state-level) and
Dataset~4 (county-level) -- to show that Drill's federation model applies just as
naturally within a single storage plugin as it does across plugins:

\begin{lstlisting}[style=sqlstyle, caption={Q6 -- state + county, both on dfs, two different files.}]
WITH s AS (
    SELECT state, SUM(CAST(NULLIF(inpatient_beds_used_covid, '') AS BIGINT)) AS covid_beds_used
    FROM dfs.healthdata.`covid/hospital_capacity_clean.csv`
    WHERE `date` = '2021/07/09'
    GROUP BY state
)
SELECT s.state,
       s.covid_beds_used   AS state_covid_beds,
       c.county,
       c.cases_last_7_days AS county_cases_7d
FROM s
JOIN dfs.healthdata.`county/community_profile.csv` c
  ON s.state = c.state
ORDER BY state_covid_beds DESC, county_cases_7d DESC
LIMIT 30
\end{lstlisting}
This query returns 30 rows in a median 0.86\,s over 3 runs and pairs each state's aggregate bed usage with
every county inside it, ordered so that the state under the most pressure (and, within
it, the counties with the most recent cases) appear first.


\section{The Jupyter Notebook and Results}
\label{sec:notebook}

\subsection{Why REST, not JDBC}
The notebook drives every query in this tutorial through Drill's REST API
(\texttt{POST~/query.json}) rather than a JDBC or ODBC driver. This keeps the Jupyter
container's dependencies to just \texttt{requests} and \texttt{pandas}, and it makes
timing straightforward. Wall-clock alone, though, only measures the round trip as
seen from the client; it cannot say how much of that time Drill spent
\emph{planning} the query versus actually \emph{executing} it against a backend. To
get both, \texttt{drill\_query} runs each query 3 times, keeps the \emph{median}
wall-clock time (so a single slow outlier does not skew the log), and then fetches
Drill's own server-side profile for the last run
(\texttt{GET~/profiles/\{queryId\}.json}), which reports three phase boundaries
(\texttt{start}, \texttt{planEnd}, \texttt{queueWaitEnd}, \texttt{end}, all epoch
milliseconds) that separate planning time, time spent queued, and actual execution
time -- server-side numbers, not just the REST round trip.

\begin{lstlisting}[style=pystyle, caption={The query helper used by every cell in analysis.ipynb.}]
import time
import statistics
import requests
import pandas as pd

DRILL_REST = "http://drill:8047/query.json"
DRILL_PROFILE = "http://drill:8047/profiles/{}.json"

def drill_query(sql: str, label: str = None, group: str = None, repeats: int = 3):
    """execute a SQL query `repeats` times, return the result as a DataFrame; log the
    median wall-clock time plus Drill's own planning/queued/execution breakdown"""
    payload = {"queryType": "SQL", "query": sql}
    wall_times = []
    data = None

    for _ in range(repeats):
        init = time.perf_counter()
        r = requests.post(DRILL_REST, json=payload, timeout=120)
        wall_times.append(time.perf_counter() - init)

        r.raise_for_status()  # raise an exception if the request failed
        data = r.json()

        if data.get("queryState") == "FAILED":
            raise RuntimeError(f"ERROR: {data}")

    profile = requests.get(DRILL_PROFILE.format(data["queryId"]), timeout=30).json()
    planning_ms = profile["planEnd"] - profile["start"]
    queued_ms = profile["queueWaitEnd"] - profile["planEnd"]
    execution_ms = profile["end"] - profile["queueWaitEnd"]

    df = pd.DataFrame(data.get("rows", []), columns=data.get("columns", []))
    median_seconds = statistics.median(wall_times)

    timings.append({
        "query": label, "group": group, "seconds": median_seconds, "rows": len(df),
        "planning_ms": planning_ms, "queued_ms": queued_ms, "execution_ms": execution_ms,
    })

    print(f"{label} median={median_seconds:.3f}s (n={repeats}) -- {len(df)} rows "
          f"[plan {planning_ms}ms / queue {queued_ms}ms / exec {execution_ms}ms]")
    return df
\end{lstlisting}

Every query cell in this tutorial calls \texttt{drill\_query(sql, label=\ldots,
group=\ldots)}; the \texttt{timings} list it appends to is what produces
Table~\ref{tab:timings}, Figure~\ref{fig:timings-by-category} and
Figure~\ref{fig:timing-breakdown} below.

\subsection{Full Results Summary}
Table~\ref{tab:timings} reproduces the notebook's complete timing log for a full
sequential run of every cell -- from the initial sanity check through Query~6 -- with
its category, median seconds over 3 repeats, returned row count, and Drill's own
server-side planning/queued/execution breakdown for the final repeat, in
milliseconds.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{@{}l l r r r r r@{}}
\toprule
\textbf{Query} & \textbf{Category} & \textbf{Sec.} & \textbf{Rows} & \textbf{Plan (ms)} & \textbf{Queued (ms)} & \textbf{Exec (ms)} \\
\midrule
sanity-check               & setup           & 0.139 & 1     & 27  & 0 & 24   \\
Q1 dfs-csv                 & single-source   & 1.020 & 20    & 233 & 0 & 514  \\
Q1b dfs-parquet            & single-source   & 0.605 & 20    & 228 & 0 & 187  \\
Q2 pg                      & single-source   & 0.846 & 20    & 99  & 0 & 499  \\
Q3 mongo                   & single-source   & 0.274 & 20    & 173 & 0 & 62   \\
QP point-lookup            & single-source   & 0.094 & 1     & 31  & 0 & 7    \\
Q4a dfs+pg                 & 2-way-join      & 1.913 & 51    & 279 & 1 & 1{,}302 \\
Q4b dfs+mongo              & 2-way-join      & 0.762 & 52    & 232 & 0 & 458  \\
Q4c pg+mongo               & 2-way-join      & 1.048 & 51    & 172 & 0 & 895  \\
Q4d dfs+pg+mongo           & 3-way-join      & 1.772 & 51    & 541 & 1 & 1{,}041 \\
Q4f dfs+pg+mongo row-level & row-level-join  & 2.522 & 1{,}581 & 299 & 0 & 1{,}590 \\
Q6 dfs-multi-scale         & other           & 0.857 & 30    & 255 & 0 & 465  \\
\bottomrule
\end{tabular}
\caption{Complete timing log for one full run of the notebook, median of 3 repeats
per query. Planning/queued/execution columns come from Drill's own
query profile for the last of the 3 repeats.}
\label{tab:timings}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{img/timings_by_category.png}
\caption{Median execution time (3 repeats) for every query in the notebook, colored by category.}
\label{fig:timings-by-category}
\end{figure}

Reading Table~\ref{tab:timings} and Figure~\ref{fig:timings-by-category} together,
three factors visibly drive latency in this federated setup: \emph{how many sources}
a query touches (single-source queries are consistently the fastest), \emph{whether
the join is pre-aggregated or row-level} (Q4f's jump to 1{,}581 rows dominates the
whole chart), and, more subtly, \emph{which} single source is queried: \texttt{mongo}
(Q3) is faster than \texttt{dfs} (Q1) here.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{img/timing_breakdown.png}
\caption{Share of total time spent planning, queued, and executing, per query, from
Drill's own profile (\texttt{GET~/profiles/\{queryId\}.json}).}
\label{fig:timing-breakdown}
\end{figure}


Figure~\ref{fig:timing-breakdown} splits each query's total time into the three
phases Drill's profile reports. \texttt{queued\_ms} is 0--1\,ms across the board:
with a single Drillbit and no concurrent workload, no query ever waits for
resources, so that phase is invisible on this setup and would only become
interesting under concurrent load or a resource-constrained cluster. The
planning/execution split, on the other hand, is not uniform, and it tracks how much
actual work a query does against its backend rather than how many sources it
touches: the sanity check (53\% planning) and QP's single-row point lookup (82\%
planning) are so cheap to execute that Drill's own query planning dominates their
total time, while every join and Q1's full-CSV scan spend the clear majority of
their time in execution (65--84\%), because there is simply more data to read,
cast, and shuffle once the query starts running. Query~1b (Section~\ref{sec:q1b})
sits in between, and interestingly so: its planning cost is nearly identical to
Query~1's (228\,ms vs.\ 233\,ms, the same query plan either way, same predicates and
grouping) but its execution cost is little more than a third of Query~1's
(187\,ms vs.\ 514\,ms), the CSV-versus-Parquet gap from Section~\ref{sec:q1b}
reappears here, isolated to the execution phase alone, exactly where a
columnar, typed format should make a difference. This is the reason
Table~\ref{tab:timings}'s seconds column and this breakdown are not redundant: a
query can be ``slow'' either because Drill takes a while to decide how to run it,
or because running it is genuinely expensive, and only the server-side profile
tells the two apart.


\section{Discussion}
\label{sec:discussion}

\subsection{Problems Encountered and How They Were Solved}
\begin{description}[leftmargin=1.6em]
  \item[Thousands-separator commas.] Dataset~1's numeric columns are formatted as
    plain text with commas (\texttt{"1,338"}), which fails a direct \texttt{CAST}.
    The fix is entirely inside the query: \texttt{REPLACE(col, ',', '')} first, then
    \texttt{NULLIF(\ldots, '')} to turn any now-empty string into \texttt{NULL}
    instead of an error, then \texttt{CAST(\ldots\ AS BIGINT)}.
  \item[\texttt{NULL} sorts first in \texttt{ORDER BY \ldots DESC}.] Rows with a
    \texttt{NULL} result (from the fix above, wherever the source value was missing)
    would otherwise appear \emph{ahead of} every real number under a descending sort,
    silently pushing the results a reader actually wants off the visible
    \texttt{LIMIT}. The fix is an explicit outer \texttt{WHERE col IS NOT NULL} after
    the aggregation, as shown in Query~1 (Section~\ref{sec:basic-queries}).
  \item[Heterogeneous date types across sources.] \texttt{dfs} represents dates as a
    plain string (\texttt{yyyy/MM/dd}); \texttt{pg} represents the same concept as a
    native \texttt{DATE}. A row-level join across the two (Query~4f) has to bridge
    that gap explicitly with \texttt{TO\_DATE(h.\`date\`, 'yyyy/MM/dd') =
    e.collection\_date} in the join predicate -- Drill does not implicitly convert
    between the two.
  \item[The storage-plugin override file was silently never applied.] For most of
    this project's development, \texttt{drill-config/storage-plugins.json} was
    mounted into the container as\\ \texttt{storage-plugins-override.json}. Drill only
    ever looks for a file named \\ exactly \texttt{storage-plugins-override.conf} (HOCON
    format), so that mount was inert: every restart quietly re-seeded Drill with its
    own bootstrap plugins (\texttt{cp}, \texttt{dfs} pointed at the container root) and
    silently dropped \texttt{pg} and \texttt{mongo}: no error, no warning, in no log
    file. It surfaced as queries against \texttt{pg}/\texttt{mongo} failing with an
    empty \texttt{FAILED} response and no message at all, and was only found by
    inspecting Drill's own local persistent store:
    (\texttt{/tmp/drill/sys.storage\_plugins/} inside the container) directly and
    noticing the custom plugins were never written there. The fix (Section~\ref{sec:plugins})
    was renaming the file and the mount destination to \texttt{.conf}.
\end{description}

\subsection{Limits of This Setup}
This is a single-node, single-machine deployment intended for teaching, not for
demonstrating true distributed scale: dataset sizes are in the tens of megabytes, well
within what a single Drill process handles comfortably, and nowhere near the volumes
that would require a multi-node Drillbit cluster. The federated joins in
Section~\ref{sec:federated-join} also make visible a genuine architectural limit of
cross-system federation: Drill can push filters and aggregations \emph{down} into a
single plugin (e.g.\ the \texttt{GROUP BY state} inside each Query~4a--4d subquery is
computed close to the data), but it cannot push the \emph{join itself} down across two
different storage systems -- the join always executes inside Drill's own execution
engine, in memory, once both sides have been scanned. That is precisely why Query~4f's
row-level join is so much more expensive than the pre-aggregated ones: pre-aggregating
first minimizes how many rows Drill has to shuffle and match itself.


\section{Conclusions}
\label{sec:conclusions}
Over the course of this tutorial we \textbf{brought up a four-service Docker environme}nt from
scratch, \textbf{loaded the same pandemic data} differently depending on its shape: raw on
the filesystem, normalized in PostgreSQL, as flexible documents in MongoDB, and
\textbf{configured Drill to see all three at once}. We verified each source independently,
then built the federated join incrementally, from pre-aggregated two-source joins up
to a full three-way, row-level join, timing every step along the way. 
That incremental build-up also surfaced Drill's real cost, not just its convenience:
pushing filters and aggregations down into a single plugin is cheap, but the join
\emph{across} plugins always happens in Drill's own memory, so its cost scales with
row count rather than with any backend's optimizer. Query~4f's row-level join, about
1.4$\times$ Query~4d's runtime over roughly 30$\times$ the rows, makes that limit
concrete rather than theoretical: row count, not join arity, is what actually drives
the cost of federation here. Timing every query as the median of 3 repeats, and
cross-checking that against Drill's own server-side planning/queued/execution
breakdown (Section~\ref{sec:notebook}), rules out a one-off slow run as the
explanation and confirms the gap is real execution cost, not noise.
\newpage
\appendix
\section{Appendix}
\label{app:extra}

\subsection{Command Log by Section}
\texttt{commands.txt} is the project's manual command log, organized into lettered
sections, and every command in it is reproduced somewhere in this report rather than
summarized: \textbf{Section~A} (Environment) is the JDBC driver
\texttt{curl} command in Section~\ref{sec:setup}. \textbf{Section~B} (Download and
Organization) is the dataset URLs and the \texttt{mkdir}/\texttt{cp}/\texttt{mv}
organization script in Section~\ref{sec:download-datasets}. \textbf{Section~C}
(Normalization and Loading) is \texttt{scripts/normalize\_columns.py}
(Section~\ref{sec:data-prep}) followed by the PostgreSQL and MongoDB loading commands
in Section~\ref{sec:loading}, each ending in a row-count verification. Anyone copying
this project can follow the report top to bottom and run every command as shown,
without needing to open \texttt{commands.txt} separately.

\subsection{Project Directory Structure}
\begin{lstlisting}[style=bashstyle]
Apache-Drill-Tutorial/
  commands.txt                 manual command log (sections A, B, C, ...)
  docker-compose.yml           drill, postgres, mongo, jupyter (+ optional admin UIs)
  data/                        gitignored; raw + cleaned datasets
  scripts/                     inspect_datasets.py, normalize_columns.py, csv_to_parquet.py
  init-sql/                    01_load_estimated_beds.sql
  init-mongo/                  01_load_therapeutic.py
  drill-config/                storage-plugins.conf
  jdbc-drivers/                postgresql-42.7.3.jar
  queries/                     demo_queries.sql
  notebooks/                   analysis.ipynb
  report/                      this document and its images
\end{lstlisting}
\subsection{GitHub Repository}
All the code can be accessed through the dedicated \href{https://github.com/DomenicoVillari3/Apache-Drill-Tutorial}{GitHub Repository}.


\end{document}
